...
public partial class HDevelopExport
{
  ...
  public void get_example_inference_images (HTuple hv_ImageDir, out HTuple hv_ImageFiles)
  {
    ...
    try
    {
      ...
      //ImageFilesIndices := [ImageFilesIndices,121, 59]
      //bags containing only missing pills.
      //ImageFilesIndices := [ImageFilesIndices,178, 18]
      //bags containing both, duplicates and missing pills
      //ImageFilesIndices := [ImageFilesIndices,146, 294]

      //═╝╞м╨Є║┼
      hv_ImageFilesIndices.Dispose();
      hv_ImageFilesIndices = new HTuple();
      hv_ImageFilesIndices[0] = 1;
      hv_ImageFilesIndices[1] = 2;
      hv_ImageFilesIndices[2] = 3;
      hv_ImageFilesIndices[3] = 4;
      hv_ImageFilesIndices[4] = 5;
      //═╝╞м┬╖╛╢
      //'/pill_bag_' + ImageFilesIndices[Index]$'03' + '.png'
      HTuple end_val19 = hv_NumSamples-1;
      HTuple step_val19 = 1;
      for (hv_Index=0; hv_Index.Continue(end_val19, step_val19); hv_Index = hv_Index.TupleAdd(step_val19))
      {
        if (hv_ImageFiles == null)
          hv_ImageFiles = new HTuple();
        hv_ImageFiles[hv_Index] = (((hv_ImageDir+"/")+(hv_ImageFilesIndices.TupleSelect(
            hv_Index)))+"_Color")+".png";
      }
  ...
  private void action()
  {
    ...
    HObject ho_ImageBatch, ho_DepthImage;
    ...
    HTuple hv_RetrainedModelFileName = new HTuple(), hv_DataDirectory = new HTuple();
    ...
    HTuple hv_DLDataInfo = new HTuple(), hv_ClassNames = new HTuple();
    HTuple hv_ClassIDs = new HTuple(), hv_DLSampleBatch = new HTuple();
    HTuple hv_DLResultBatch = new HTuple(), hv_SampleIndex = new HTuple();
    HTuple hv_DLSample = new HTuple(), hv_DLResult = new HTuple();
    HTuple hv_DetectedClassIDs = new HTuple(), hv_NumberDetectionsPerClass = new HTuple();
    HTuple hv_Index = new HTuple(), hv_bbox_row = new HTuple();
    HTuple hv_bbox_col = new HTuple(), hv_cx = new HTuple();
    HTuple hv_cy = new HTuple(), hv_Pose_CamToTool = new HTuple();
    HTuple hv_u = new HTuple(), hv_v = new HTuple(), hv_depth = new HTuple();
    HTuple hv_fx = new HTuple(), hv_fy = new HTuple(), hv_X_camera = new HTuple();
    HTuple hv_Y_camera = new HTuple(), hv_Z_camera = new HTuple();
    HTuple hv_X = new HTuple(), hv_Y = new HTuple(), hv_Z = new HTuple();
    HTuple hv_Rx = new HTuple(), hv_Ry = new HTuple(), hv_Rz = new HTuple();
    HTuple hv_Pose_BaseToTool = new HTuple(), hv_Pose_ToolToCam = new HTuple();
    HTuple hv_Pose_BaseToCam = new HTuple(), hv_HomMat3D_BaseToCam = new HTuple();
    HTuple hv_Xb = new HTuple(), hv_Yb = new HTuple(), hv_Zb = new HTuple();
    // Initialize local and output iconic variables 
    HOperatorSet.GenEmptyObj(out ho_ImageBatch);
    HOperatorSet.GenEmptyObj(out ho_DepthImage);
    try
    {
      //│ї╩╝╔ш╓├
      //гиpathгй╩ф╚ы═╝╞м┬╖╛╢
      hv_ImageDir.Dispose();
      hv_ImageDir = "E:/Halcon/Get_item_location/inputImages";
      //(path)─г╨═╩¤╛▌┬╖╛╢
      hv_ExampleDataDir.Dispose();
      hv_ExampleDataDir = "E:/Halcon/Get_item_location";
      //
      //(bool)╩╟╖ё╩╣╙├╘д╤╡┴╖─г╨═
      ...
      if ((int)(hv_UsePretrainedModel) != 0)
      {
        ...
        hv_PreprocessParamFileName = "model_test1.hdict";
        //╘д╤╡┴╖─г╨═┬╖╛╢hdl
        hv_RetrainedModelFileName.Dispose();
        hv_RetrainedModelFileName = "model_test1.hdl";
      }
      else
      {
      ...
      //┐╔╩╙╗п═и╙├▓╬╩¤.
      //бкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбк


      //═╞└э╟°
      ho_ImageBatch.Dispose();
      HOperatorSet.ReadImage(out ho_ImageBatch, "E:/Halcon/Get_item_location/inputImages/1_Color.png");
      //
      ...
      //бкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбкбк
      //╝╞╦у╟°
      //╗ё╚б ╡┌╥╗╕Ў ╬я╠х ╓╨╨─╡у ╧ё╦╪╫°▒ъ
      hv_bbox_row.Dispose();
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      ...
      //╗ё╚б╛р└ы═╝╞м╓╨╨─╛р└ы╫ю╜№╡──┐▒ъ╫°▒ъгиDLResult.bbox_rowгмDLResult.bbox_colгй
      ...
      HOperatorSet.ReadPose("movingcam_final_pose_cam_tool.dat", out hv_Pose_CamToTool);

      //╫°▒ъгм╡е╬╗╧ё╦╪
      hv_u.Dispose();
      hv_u = new HTuple(hv_bbox_row);
      hv_v.Dispose();
      hv_v = new HTuple(hv_bbox_col);

      //╢┴╚бtiff╬─╝■╗ё╚б╔ю╢╚гм╡е╬╗mm
      ho_DepthImage.Dispose();
      HOperatorSet.ReadImage(out ho_DepthImage, "E:/Halcon/Get_item_location/inputImages/1_Depth.Tiff");
      hv_depth.Dispose();
